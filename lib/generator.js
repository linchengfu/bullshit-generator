...
import { createRandomPicker, randomIndex } from "./random.js";
...
export function generate(title, { corpus, min = 6000, max = 10000 }) {
  const articleLength = randomIndex(min, max);
  ...
  while (totalLength < articleLength) {
    let section = "";
    const sectionLength = randomIndex(200, 500);
    while (section.length < sectionLength || !/[。？]$/.test(section)) {
      const n = randomIndex(0, 100)
      if (n < 20) {
